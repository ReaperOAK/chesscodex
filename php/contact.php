...
if (!empty($role) && !empty($name) && !empty($age) && filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($phone_number) && !empty($message) && $agree_privacy_policy) {
    // Prepare the SQL statement for contact submissions
    $stmt = $conn->prepare("INSERT INTO contact_submissions (role, name, age, email, phone_number, message, subscribe_newsletter, agree_privacy_policy) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        echo json_encode(['success' => false, 'message' => 'Prepare failed: ' . $conn->error]);
        exit;
    }
    $stmt->bind_param("ssisssii", $role, $name, $age, $email, $phone_number, $message, $subscribe_newsletter, $agree_privacy_policy);
